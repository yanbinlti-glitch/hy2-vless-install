...
def msg(s):
    print("[safe-patch-v2]", s, flush=True)
...
def rollback(reason):
    print("\n[safe-patch-v2] 补丁失败，开始回滚。", file=sys.stderr)
    print("[safe-patch-v2] 失败原因:", reason, file=sys.stderr)
    if ORIG:
        run(["git", "reset", "--hard", ORIG], check=False)
        run(["git", "reset"], check=False)
        print("[safe-patch-v2] 已回滚到:", ORIG, file=sys.stderr)
    if BACKUP:
        print("[safe-patch-v2] 保留备份分支:", BACKUP, file=sys.stderr)
...
def backup():
    ...
    BACKUP = f"backup/before-menu-guard-v2-{int(time.time())}"
...
def find_func_span(text, name):
    m = re.search(rf"(?m)^{name}\(\) \{{", text)
    if not m:
        raise RuntimeError(f"未找到函数: {name}()")
    start = m.start()
    i = m.end()
    depth = 1
    while i < len(text):
        ...
        if ch == "{":
            ...
            if depth == 0:
                return start, i + 1
        i += 1
    raise RuntimeError(f"函数括号不完整: {name}()")
...
EOF_WRAPPER
 ...
 chmod +x /usr/bin/666
    ...
    s, e = find_func_span(t, "install_self_shortcut")
    t = t[:s] + new_shortcut + t[e:]

    bottom_pat = re.compile(
        r'ensure_modules \|\| exit 1\s+'
        r'validate_local_bundle \|\| exit 1\s+'
        r'for module in "\$\{MODULES\[@\]\}"; do\s+'
        r'# shellcheck source=/dev/null\s+'
        r'source "\$SCRIPT_DIR/lib/\$module"\s+'
        r'done\s+'
        r'install_self_shortcut\s+'
        r'while true; do\s+'
        r'menu\s+'
        r'done',
        re.M
...
while true; do
 menu
    ...
    t, n = bottom_pat.subn(new_bottom, t, count=1)
    if n != 1:
        raise RuntimeError("未能替换底部主流程。")
